...
<div class="wc_donaite_header">
    <div class="statistics">
        <div class="td">
            <?php
            $currency = cupri_get_currency();
            $current_currency = cupri_get_currency_value();
            $current_currency = strtolower($current_currency);
            if ($current_currency == 'rial' || $current_currency == 'toman') {
                $currency = _e('Toman', 'cupri');
            }

            ?>
            <span class="counter total"><?php echo number_format(cupri_get_total_payments()); ?> <small
                        class="counter_desc"><?php echo esc_html($currency); ?></small></span>
            <span class="counter_desc"><?php _e('Total successful payments', 'cupri'); ?></span>
        </div>
        <?php
        /*
        <div class="td">
